...
When upgrading KernelSU, keep the installed Manager, userspace daemon, and kernel component from the same release whenever possible. The Manager checks the kernel UAPI version at startup and may require a newer kernel if the userspace-kernel interface is not compatible.
