...
This constraint also applies during upgrades. To switch mounting implementations, remove the active metamodule first, reboot, then install the replacement. Keeping two metamodules installed is not supported and can leave regular modules without a predictable mount provider.
