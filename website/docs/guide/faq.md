...
## Why does the Manager say that I need a newer kernel?

KernelSU's Manager checks whether the kernel UAPI matches the Manager it was built with. If the kernel, `ksud`, and Manager come from different builds or releases, the Manager may block some features and ask you to upgrade the kernel. Install matching components from the same release whenever possible.
